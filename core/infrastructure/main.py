...
class TelloByJenia:
    ...
    state = state_field_converters.update({key: float for key in FLOAT_STATE_FIELDS})
    ...
    is_flying = False
    host = TELLO_IP
    retry_count = RETRY_IF_FAIL_COUNT

    def __post_init__(self) -> None:

        global threads_initialized, client_socket, drones
        self.last_received_command_timestamp = time.time()
        self.address = (self.host, Tello.CONTROL_UDP_PORT)
        ...
        drones[self.host] = {'responses': [], 'state': {}}

        log(f"Tello instance was initialized. Host: '{self.host}'. Port: {Tello.CONTROL_UDP_PORT}", level=logging.INFO)
    ...
    def send_command_with_return(self, command: str, timeout: int = RESPONSE_TIMEOUT) -> str:
        ...
        if diff < TIME_BTW_COMMANDS:
            log(f'Waiting {diff} seconds to execute command: {command}...', logging.DEBUG)
            time.sleep(diff)

        log(f"Send command: '{command}'")
        timestamp = time.time()
        ...
        log("Response {}: '{}'".format(command, response))
        return response
    ...
    def send_command_without_return(self, command: str):
        ...
        log("Send command (no response expected): '{}'".format(command))
        client_socket.sendto(command.encode('utf-8'), self.address)
    ...
    def send_control_command(self, command: str, timeout: int = RESPONSE_TIMEOUT) -> bool:
        ...
        for i in range(0, self.retry_count):
            ...
            log(text=f"Command attempt #{i} failed for command: {command}", level=logging.DEBUG)
    ...
    def send_read_command(self, command: str) -> str:
        ...
        try:
            ...
            log(text=e, level=logging.ERROR)
    ...
    def streamon(self):
        ...
        when your computer is connected to Tello-XXXXXX Wi-Fi ntwork).
    ...
    def curve_xyz_speed_mid(self, x1: int, y1: int, z1: int, x2: int, y2: int, z2: int, speed: int, mid: int):
        ...
        Arguments:
            ...
            mid: 1-8
        """
        cmd = f'curve {x1} {y1} {z1} {x2} {y2} {z2} {speed} m{mid}'
        self.send_control_command(cmd)
    ...
    def go_xyz_speed_yaw_mid(self, x: int, y: int, z: int, speed: int, yaw: int, mid1: int, mid2: int):
        ...
        Arguments:
            ...
            mid2: 1-8
        """
        cmd = f'jump {x} {y} {z} {speed} {yaw} m{mid1} m{mid2}'
        self.send_control_command(cmd)
    ...
    def send_rc_control(self, left_right_velocity: int, forward_backward_velocity: int, up_down_velocity: int,
        ...
        if time.time() - self.last_rc_control_timestamp > TIME_BTW_RC_CONTROL_COMMANDS:
